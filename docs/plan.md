...
| 6 | 約物のアキは中間状態で固定する               | 全角ベタは緩く、`palt` 全開は詰まりすぎる。中間は CSS では表現できない  |
...
| 項目               | 判断                                                                  |
| ------------------ | --------------------------------------------------------------------- |
| 縦組み             | 横書きのみ対応。`vert` / `vrt2` / `vpal` / `vhal` 等の検証・調整は行わない |
| 印刷・アプリ用途   | Web 配信（woff2）に絞る。ヒンティングや OTF/TTF での配布は扱わない      |
| 和欧間のアキ       | CSS の `text-autospace` に委ねる（→ 6）                                |
| `chws` の実装      | 実装済みブラウザが限られ、実効性がないため不採用（→ 4.2）               |
| 可変フォント出力   | 合成方式の帰結として静的のみとする                                     |

縦組み用の feature は削除までは行わず、Noto Sans JP 由来のものが残る場合も検証対象外と
して扱う。
...
Noto Sans JP の `palt` は `GPOS` の `SinglePos` として約物に advance と placement の
調整を持つ。これを比率 `f` で分割し、`f` 分を `hmtx` とアウトラインに焼き込み、
残り `1 − f` を `palt` に書き戻す。

結果として、既定状態が全角ベタと `palt` 全開の中間になり、CSS の指定なしにすべての
ブラウザで同一の結果が得られる。`palt` を有効化すれば従来どおり全開まで到達する。

同種の目的を持つ `chws` は採用しない。仕様上は JLREQ 相当を実装していないエンジンが
既定で有効にすべき feature だが、実際に適用されるのは Chromium 系に限られ、WebKit /
Gecko では効かない。一方 `palt` は3エンジンすべてで動作するため、焼き込み方式のほうが
確実である。
...
| 字送り       | 和文の advance には手を入れない（②の約物を除く）                         |
...
| #  | マイルストーン     | 完了条件                                                            |
| -- | ------------------ | ------------------------------------------------------------------- |
| M1 | 合成の実現性検証   | 1ウェイトで③を通し、グリフ名衝突・cmap・GSUB/GPOS の統合方式を確定    |
...
M2 までに確定する。値と根拠は別ドキュメントに記録する。

| # | 項目                        | 内容                                            |
| - | --------------------------- | ----------------------------------------------- |
| 1 | ウェイト数                  | Noto Sans JP 側の主要ウェイトを網羅しつつ、少ないほどビルドが軽い |
| 2 | 欧文の `wght` インスタンス化値 | 和文と見かけの太さが揃う位置（和文はラウンドな値で固定）        |
| 3 | `palt` の焼き込み比率       | 0〜100%。既存サイトの字送りへの影響と併せて判断                 |
| 4 | `SCALE` / `BASELINE_OFFSET` | 欧文を和文に合わせる値。cap-height / x-height 比から算出        |
...
| #  | リスク                                            | 影響 | 対策                                                                 |
| -- | ------------------------------------------------- | ---- | -------------------------------------------------------------------- |
| R1 | 合成時のグリフ名衝突・cmap マージ・GSUB/GPOS 統合 | 高   | M1 として最初に検証。fontTools の `merge` で不足する場合は他のツールを検討 |
| R2 | サブセット化で layout feature が脱落する          | 中   | ⑤の後に feature 保持の検証工程を設ける                                |
| R3 | ウェイトによって `palt` の値が変わり、太い側で急に詰まる | 中   | 全ウェイトで `palt` のカバレッジと値を統一する                        |
| R4 | ウェイトごとに `glyphYShift` の最適値が異なる     | 低   | 静的インスタンス化後に適用し、ウェイト別に値を持てる構造とする        |
| R5 | ②の約物焼き込みが既存サイトのレイアウトを動かす   | 中   | 置き換え互換性と字送りの改善はトレードオフとなる。焼き込み比率を M2 で判断し、M4 で影響を確認 |
