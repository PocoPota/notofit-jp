...
├── 100.css 〜 900.css  ウェイト単位で読み込みたい場合
...
| M5 | 全ウェイト展開     | **完了**（2026-07-28）。`notofit.release` が 100〜900 の9ウェイトを一括で生成する |
...
**ウェイトは 100〜900 の9段すべてとする（確定）。** 設計値は目視で決めた 400 / 700 / 800 を
基準に、和文の線幅と cap 高さの関係から残りを求めている。
...
| 2 | 欧文の `wght` インスタンス化値 | 和文と見かけの太さが揃う位置（和文は CSS 上の値と同じ wght で固定） |
...
| R3 | `palt` の値がウェイトの境界で切り替わり、字送りに段差が出る | 低   | **既知の挙動として受け入れる**。100–600 と 700–900 がそれぞれ同一値で、600–700 間にのみ段差がある（本文サンプルで +0.87%）。解消するには全ウェイトの `palt` を片側に揃えることになり、公開済みの 400 / 700 の字送りが動く（[notes/palt-weight-boundary.md](notes/palt-weight-boundary.md)） |
