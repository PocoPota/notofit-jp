...
2つの異なる処理を行う。詳細と決定経緯は [yakumono.md](yakumono.md)。
...
**かな・全角英数** — `palt` を焼き込んでプロポーショナル化する。比率の分を `hmtx` と
アウトラインに恒久的に書き込み、残りを `palt` に書き戻す。字面が締まる一方で体感の
フォントサイズは変わらないため、置き換えの妨げにならない。
...
| M2 | 設計値の確定       | 下記の決定事項をすべて数値として確定                                 |
| M3 | 単一ウェイトの完成 | ①〜⑥ を通して1ウェイト分を生成し、3エンジンで横書き表示を確認        |
| M4 | 置き換え互換性の確認 | Noto Sans JP 採用ページで差し替え、行送り・折り返し位置の一致を確認  |
...
**設計値は目視で判断する。** cap-height 比などの数値は出発点および参考として用い、
最終的な採否は和欧を並べた見え方で決める。この作業のために調整ツールを用意する
（→ [tuner.md](tuner.md)）。
...
| 3 | 約物の `kern` 行列と `palt` 焼き込み比率 | 隣接時のアキ量と、かな・全角英数の縮み具合             |
| 4 | `excludeCodepoints` の対象  | CJK の字形を残す記号の選定                              |
| 5 | `glyphYShift` の対象と値    | Outfit のコロン類ほか、上下位置を補正するグリフ         |
| 6 | `metadataMode`              | name テーブルの識別情報をどう構成するか                 |
