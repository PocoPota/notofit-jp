...
| 項目 | 値 |
| ---- | -- |
| ウェイト | 100 / 200 / 300 / 400 / 500 / 600 / 700 / 800 / 900 の9段 |
| スタイル | ローマン体のみ。イタリックはない |
| 書字方向 | **横書き専用**。縦組みは対象外 |
| 形式 | woff2 のみ。TTF / OTF は配布しない |
| units per em | 1000 |
| スライス | 125（`unicode-range` による分割）× ウェイト = 1,125 ファイル |
| パッケージ | 約 26 MB。1ページが読むのは使用文字ぶんの十数ファイル |
...
位置は変わる**。詰まる量はウェイトによってわずかに違い、100〜600 と 700〜900 の間に
約 0.9% の差がある（Noto Sans JP の `palt` がこの境界で切り替わるため）。
